...
class test_discrete_logistic(unittest.TestCase):
    ...
    def test_log_prob(self):
        ...
        #check values in (-1, 1) return the discretized log_prob
        values = (-0.999, -0.6, -0.3, 0., 0.3, 0.6, 0.999)
        ...
        #check values <= -1 are given the remaining density for values in (-inf, -1]
        values = (-2., -1.5, -1.0)
        ...
        #check values >= 1 are given the remaining density for values in [1, inf)
        values = (1.0, 1.5, 2.0)
        ...
        #closed form probability density function for logistic distribution
